					  ...
					  uint64_t *FolderId)
{
	...
	DEBUG(priv_data->log_level, ("%s%s[in]: system_idx=[0x%08"PRIx32"]\n",
					priv_data->log_prefix, __FUNCTION__, SystemIdx));
	retval = priv_data->backend->get_PublicFolderID(priv_data->backend, SystemIdx, FolderId);
	DEBUG(priv_data->log_level, ("%s%s[out]: retval=[%s], folder_id=[0x%016"PRIx64"]\n",
					priv_data->log_prefix, __FUNCTION__, mapi_get_errstr(retval), *FolderId));
					       ...
					       struct GUID *ReplGUID)
{
	...
	retval = priv_data->backend->get_PublicFolderReplica(priv_data->backend, ReplID, ReplGUID);
	DEBUG(priv_data->log_level, ("%s%s[out]: retval=[%s]\n",
					priv_data->log_prefix, __FUNCTION__, mapi_get_errstr(retval)));
...
static enum MAPISTATUS get_new_changeNumber(struct openchangedb_context *self, uint64_t *cn)
{
	...
	DEBUG(priv_data->log_level, ("%s%s[in]: \n",
					priv_data->log_prefix, __FUNCTION__));
	retval = priv_data->backend->get_new_changeNumber(priv_data->backend, cn);
	DEBUG(priv_data->log_level, ("%s%s[out]: retval=[%s]\n",
					priv_data->log_prefix, __FUNCTION__, mapi_get_errstr(retval)));
					     ...
					     struct UI8Array_r **cns_p)
{
	...
	retval = priv_data->backend->get_new_changeNumbers(priv_data->backend, mem_ctx, max, cns_p);
					     ...
					     uint64_t *cn)
{
	...
	retval = priv_data->backend->get_next_changeNumber(priv_data->backend, cn);
				      ...
				      uint64_t fid, const char *display_name)
{
	...
	DEBUG(priv_data->log_level, ("%s%s[in]: username=[%s]\n",
					priv_data->log_prefix, __FUNCTION__, username));
	retval = priv_data->backend->create_mailbox(priv_data->backend, username, systemIdx, fid, display_name);
	DEBUG(priv_data->log_level, ("%s%s[out]: retval=[%s]\n",
					priv_data->log_prefix, __FUNCTION__, mapi_get_errstr(retval)));
...
static const char *get_indexing_url(struct openchangedb_context *self, const char *username)
{
	...
	DEBUG(priv_data->log_level, ("%s%s[in]: username=[%s]\n",
					priv_data->log_prefix, __FUNCTION__, username));
	url = priv_data->backend->get_indexing_url(priv_data->backend, username);
	DEBUG(priv_data->log_level, ("%s%s[out]: url=[%s]\n",
					priv_data->log_prefix, __FUNCTION__, url));
